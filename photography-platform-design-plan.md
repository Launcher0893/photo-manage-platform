...
首页展示轮播图、精选摄影作品、推荐摄影师、热门作品和最新公告。

轮播图数据来自 `carousel` 表；精选作品来自 `photo_work.is_featured = 1` 且审核通过的作品；推荐摄影师来自审核通过的摄影师资料；公告来自 `announcement` 表中已发布的数据。
...
摄影师注册后进入待审核状态。管理员审核通过后，该摄影师才可以被展示在用户端推荐摄影师列表中。摄影师主页自动聚合该摄影师发布或由管理员维护的所有作品。
...
管理员可以查看用户列表，按用户名、昵称、角色、状态筛选用户。管理员可以禁用用户，查看用户详情和注册时间。

个人中心支持用户修改头像、修改密码、查看我的帖子。头像上传到 OSS，返回的 URL 存入 `user.avatar_url`。
...
1. OSS 密钥不能提交到 Git 仓库，正式项目应使用环境变量或 `.env` 文件管理。
...
7. 后台删除建议优先使用软删除，即将 `status` 改为 0，避免误删重要数据。
8. 第一版按课程项目难度设计为 Flask 服务端渲染，不做前后端分离。

## 13. 后续可扩展功能

1. 摄影师自行上传作品，而不是只由管理员维护。
2. 作品支持多个标签，新增 `work_category` 中间表。
3. 评论增加回复功能，支持楼中楼。
4. 用户关注摄影师，新增 `photographer_follow` 表。
5. 后台增加更细的角色权限控制。
6. 首页推荐作品根据热度和分类自动计算。
7. 增加图片压缩、水印和缩略图生成。
